...
<%--                                <td>--%>
<%--                                    <a href="${pageContext.request.contextPath}/studentInfo/update?studentId=${student.studentId}"--%>
<%--                                       class="btn1 btn-xs glyphicon glyphicon-edit">修改</a>--%>
<%--                                    <a href="#" onclick="deleteStudent(${student.studentId})"--%>
<%--                                       class="btn2 btn-xs glyphicon glyphicon-trash">删除</a>--%>
<%--                                    <a href="#" class="btn3 btn-xs glyphicon glyphicon-repeat"--%>
<%--                                       onclick="rest(${student.studentId})">重置密码</a>--%>
<%--                                </td>--%>
                            </tr>
                        </c:forEach>
                        </tbody>
                ...
                <td>
                    ...
                    <span>
						<c:if test="${requestScope.pagemsg.currPage != 1}">
                            <a style="color: black;"
                               href="${pageContext.request.contextPath }/studentInfo/query?currentPage=1&id=${requestScope.pagemsg.id}">[首页]</a>
                            <a style="color: black;"
                               href="${pageContext.request.contextPath }/studentInfo/query?currentPage=${requestScope.pagemsg.currPage-1}&id=${requestScope.pagemsg.id}">[上一页]</a>
                        </c:if> <c:if test="${requestScope.pagemsg.currPage != requestScope.pagemsg.totalPage}">
                        <a style="color: black;"
                           href="${pageContext.request.contextPath }/studentInfo/query?currentPage=${requestScope.pagemsg.currPage+1}&id=${requestScope.pagemsg.id}">[下一页]</a>
                        <a style="color: black;"
                           href="${pageContext.request.contextPath }/studentInfo/query?currentPage=${requestScope.pagemsg.totalPage}&id=${requestScope.pagemsg.id}">[尾页]</a>
                    </c:if>
					</span>
                </td>
            </tr>
