...
public class LoginServiceImpl implements LoginService
{
    ...
    public boolean studentlogin(String name, String password)
    {
        ...
        {
            System.out.println("获取学生用户名密码成功");
            return true;
        }
        System.out.println("获取学生用户名密码失败");
        return false;
    }
    ...
    {
        return loginMapper.changeUserPwd(name, newPad);
    }

    @Override
    public boolean studentRegister(String name, String password)
    {
        try
        {
            int i = loginMapper.addInfo(name, password);
            if (i > 0)
            {
                return true;
            }
